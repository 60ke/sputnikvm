    ...
    pub fn into_context<P: Patch>(
        ...
        match self.action {
            TransactionAction::Call(_) => {
                ...
                println!("transaction call value {:?} ", self.value);
                println!("upfront {:?}",upfront );
                Ok(Context {
                    address,
...
impl<M: Memory + Default, P: Patch> VM for TransactionVM<M, P> {
    ...
    fn step(&mut self) -> Result<(), RequireError> {
        ...
        println!("account_state {:?}",account_state.accounts());
        println!("vm accounts {:?}", vm.accounts());
        ...
        // println!("222222222222vm accounts {:?}", vm.accounts());
        Ok(())
    }
    ...
    fn readset(&self) -> ReadSet {
        // ReadSet::default()
        match self.0 {
            TransactionVMState::Running { ref vm, .. } => vm.machines[0].state().readset.clone(),
            TransactionVMState::Constructing { .. } => ReadSet::default(),
        }        

    }

    fn writeset(&self) -> WriteSet {
        // WriteSet::default()
        match self.0 {
            TransactionVMState::Running { ref vm, .. } => vm.machines[0].state().writeset.clone(),
            TransactionVMState::Constructing { .. } => WriteSet::default(),
        }        
